...
class ReservationController extends AbstractController
{
     ...
     */
    public function new(Request $request, Swift_Mailer $mailer, ReductionRepository $reductionRepository, HotelRepository $hotelRepository, ReservationRepository $reservationRepository, ChambreRepository $chambreRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $date = new \DateTime();
            $begin = $date->diff($newReservation->getDateArrivee());
            if ($begin->format("%a") > 1 || $interval->format("%a") > 10) {
            ...
            $allRooms = $chambreRepository->findAll();
            $oneRoom = $reservationRepository->findAll();

            if (!empty($oneRoom)) {
                for ($i = 0; $i < count($oneRoom); $i++) {
                    if ($reservation->getDateArrivee() >= $oneRoom[$i]->getDateArrivee() or $reservation->getDateDepart() <= $oneRoom[$i]->getDateDepart()) {
                        unset($allRooms[$oneRoom[$i]->getChambre()->getId() - 1]);
                    }
                }

            }
            $reservation->setChambre(reset($allRooms));
